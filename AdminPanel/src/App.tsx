...
export default function AdminPanel() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      let {data: profiles, error: profileError} = await supabase.from("profiles").select("*").eq("username", username).limit(1);
      if(!profileError && (!profiles || profiles.length === 0) && username.includes("@")) {
        const byEmail = await supabase.from("profiles").select("*").eq("email", username).limit(1);
