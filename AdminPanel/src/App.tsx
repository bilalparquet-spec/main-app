...
export default function AdminPanel() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      let {data: profiles, error: profileError} = await supabase.from("profiles").select("*").eq("email", email).limit(1);
      if(profileError) throw profileError;
      const profile = profiles?.[0];
      if(!profile || !["admin","admin_staff","superadmin"].includes(profile.role)) throw new Error("هذا الحساب لا يملك صلاحية الإدارة");
