...
export default function AdminPanel() {
  ...
  async function loadAdminData() {
    if(!supabaseReady) return;
    const [agRes,cRes,bRes,rRes,pRes,uRes,mRes] = await Promise.all([
      ...
      supabase.from("pending_requests").select("*").order("request_date",{ascending:false}),
    ...
    if(Array.isArray(mRes.data)) {
      ...
      mRes.data.forEach(m=>{
        ...
        if(!msgsByAgency[m.agency_id]) msgsByAgency[m.agency_id]=[];
        msgsByAgency[m.agency_id].push({
          id:m.id,
          from:m.direction==="agency_to_user"?"وكالة":"مستخدم",
          text:m.body||m.subject||"رسالة",
          user:m.user_name||"مستخدم",
          userId:m.user_id,
      ...
      if(Array.isArray(agRes.data)) {
        setAgencies(agRes.data.map(r=>({...mapAgencyRow(r), messages:msgsByAgency[r.id]||[]})));
      }
    } else {
    ...
    if(Array.isArray(uRes.data)) setUserRequests(uRes.data);
  }
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(async ({data:{session}})=>{
      ...
      try {
        ...
        if(profile && ["admin","admin_staff"].includes(profile.role) && profile.status==="active") {
          setAuth(adminProfileToSession(profile));
          setPage("dashboard");
          await loadAdminData();
        }
      } catch(e) {}
    ...
    const channel=supabase.channel("admin_pending_rt")
      ...
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"pending_requests"},(payload)=>{
        ...
        setUserRequests(prev=>prev.find(x=>x.id===r.id)?prev:[{
          id:r.id,username:r.username,name:r.name||r.username,
          email:r.email,phone:r.phone,avatar:r.avatar,status:"pending",requestDate:r.request_date,
        },...prev]);
        setNotifications(prev=>[{
          id:"req_"+r.id,type:"new",
          msg:"🔔 طلب تسجيل مستخدم جديد: "+(r.name||r.username),
          time:"الآن",read:false,
        },...prev]);
      })
      // طلبات تسجيل الوكالات الجديدة
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"agencies"},(payload)=>{
        const r=payload.new;
        if(r.status!=="pending") return;
        setAgencies(prev=>prev.find(a=>a.id===r.id)?prev:[mapAgencyRow(r),...prev]);
        setNotifications(prev=>[{
          id:"ag_"+r.id,type:"new",
          msg:"🏢 طلب تسجيل وكالة جديدة: "+r.name,
          time:"الآن",read:false,
        },...prev]);
      })
      // تحديث حالة الوكالات
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"agencies"},(payload)=>{
        setAgencies(prev=>prev.map(a=>a.id===payload.new.id?mapAgencyRow(payload.new):a));
      })
      // رسائل جديدة
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages"},(payload)=>{
        const r=payload.new;
        if(!r.agency_id) return;
        const newMsg={id:r.id,from:r.direction==="agency_to_user"?"وكالة":"مستخدم",text:r.body||r.subject||"رسالة",user:r.user_name||"مستخدم",userId:r.user_id,date:r.created_at?.split("T")[0]||"",time:r.created_at?.split("T")[1]?.slice(0,5)||""};
        setAgencies(prev=>prev.map(a=>a.id===r.agency_id?{...a,messages:[...(a.messages||[]),newMsg]}:a));
        setNotifications(prev=>[{
          id:"msg_"+r.id,type:"message",
          msg:"💬 رسالة جديدة من: "+(r.user_name||"مستخدم")+" إلى: "+(r.agency_name||"الوكالة"),
          time:"الآن",read:false,
        },...prev]);
      })
      // طلبات تجديد الاشتراك
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"subscription_renewal_requests"},(payload)=>{
        const r=payload.new;
        const ag=agencies.find(a=>a.id===r.agency_id);
        setNotifications(prev=>[{
          id:"ren_"+r.id,type:"renewal",
          msg:"🔄 طلب تجديد اشتراك من: "+(ag?.name||r.agency_id),
          time:"الآن",read:false,
        },...prev]);
        setAgencies(prev=>prev.map(a=>a.id===r.agency_id?{...a,renewalRequest:true,renewalMonths:r.requested_months}:a));
      })
      // حجوزات جديدة
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"bookings"},(payload)=>{
        setBookings(prev=>prev.find(b=>b.id===payload.new.id)?prev:[mapBookingRow(payload.new),...prev]);
        setNotifications(prev=>[{
          id:"book_"+payload.new.id,type:"booking",
          msg:"📋 حجز جديد: "+(payload.new.car_name||"سيارة"),
          time:"الآن",read:false,
        },...prev]);
      })
      // تعليقات جديدة للمراجعة
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"reviews"},(payload)=>{
        const r=payload.new;
        if(r.status==="pending") {
          setReviews(prev=>prev.find(x=>x.id===r.id)?prev:[mapReviewRow(r),...prev]);
          setNotifications(prev=>[{
            id:"rev_"+r.id,type:"review",
            msg:"⭐ تعليق جديد ينتظر المراجعة",
            time:"الآن",read:false,
          },...prev]);
        }
      })
  ...
  async function handleAgencyAction(id, action, value) {
    // حفظ في Supabase
    if(supabaseReady) {
      try {
        switch(action) {
          case "approve":
            await supabase.from("agencies").update({status:"active",pending_approval:false}).eq("id",id);
            await supabase.from("profiles").update({status:"active"}).eq("agency_id",id);
            break;
          case "reject":
            await supabase.from("agencies").update({status:"rejected",pending_approval:false}).eq("id",id);
            await supabase.from("profiles").update({status:"rejected"}).eq("agency_id",id);
            break;
          case "verify": await supabase.from("agencies").update({verified:true}).eq("id",id); break;
          case "unverify": await supabase.from("agencies").update({verified:false}).eq("id",id); break;
          case "suspend": await supabase.from("agencies").update({status:"suspended"}).eq("id",id); break;
          case "unsuspend": await supabase.from("agencies").update({status:"active"}).eq("id",id); break;
          case "feature": {
            const ag = agencies.find(a=>a.id===id);
            await supabase.from("agencies").update({featured:!ag?.featured}).eq("id",id);
            break;
          }
          case "setPrice": await supabase.from("agencies").update({monthly_price:value}).eq("id",id); break;
          case "setPlan": await supabase.from("agencies").update({plan:value}).eq("id",id); break;
          case "setSubscriptionDates": await supabase.from("agencies").update({subscription_start:value.start,subscription_end:value.end}).eq("id",id); break;
          case "approveRenewal": {
            const ag = agencies.find(a=>a.id===id);
            const now = new Date();
            ...
            await supabase.from("agencies").update({status:"active",subscription_start:now.toISOString().split("T")[0],subscription_end:end.toISOString().split("T")[0],renewal_request:false}).eq("id",id);
            break;
          }
          case "rejectRenewal": await supabase.from("agencies").update({renewal_request:false}).eq("id",id); break;
          case "delete": await supabase.from("agencies").delete().eq("id",id); break;
        }
      } catch(e) { console.error("Agency action error:",e); }
    }
  ...
  if(sessionLoading) return (
    <div style={{position:"fixed",inset:0,background:"#07080F",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{color:"#7C3AED",fontSize:14,fontFamily:"system-ui"}}>جاري التحميل...</div>
    </div>
  );
                ...
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  ...
                  {rev.status==="pending"&&<>
                    <button onClick={async()=>{
                        if(supabaseReady) try{await supabase.from("reviews").update({status:"approved"}).eq("id",rev.id);}catch(e){}
                        setReviews(p=>p.map(r=>r.id===rev.id?{...r,status:"approved"}:r));
                      }}
                    ...
                    <button onClick={async()=>{
                        if(supabaseReady) try{await supabase.from("reviews").update({status:"rejected"}).eq("id",rev.id);}catch(e){}
                        setReviews(p=>p.map(r=>r.id===rev.id?{...r,status:"rejected"}:r));
                      }}
                  ...
                  {rev.status!=="pending"&&<button onClick={async()=>{
                      if(supabaseReady) try{await supabase.from("reviews").delete().eq("id",rev.id);}catch(e){}
