...
function AgencyPortal({lang,agency,bookings,setBookings,convs,setConvs,userCars,setUserCars,allWilayas,onLogout}){
  ...
  // Realtime messages handled via Supabase channel
  ...
  const submitCar=async()=>{
    ...
    const carId = editingCar?editingCar.id:("car"+Date.now());
    const newCar={
      id:carId,
      agencyId:agency.id,
    ...
    // حفظ في Supabase
    const {error: carErr} = editingCar
      ? await supabase.from("cars").update({
          agency_id: agency.id,
          agency_name: agency.name||agency.ar||"",
          name: newCar.name,
          fuel: newCar.fuel,
          type: newCar.type,
          badge: newCar.badge||newCar.type,
          price: newCar.price,
          year: newCar.year,
          seats: newCar.seats,
          trans: newCar.transmission||"manual",
          available: newCar.available,
          img: newCar.img,
          image: newCar.img,
          wilaya: newCar.wilaya,
          status: "approved",
        }).eq("id",carId)
    ...
    if(carErr){ showNotif("❌ خطأ: "+carErr.message); console.error(carErr); return; }
    if(editingCar){
  ...
  const deleteCar=async id=>{
    try { await supabase.from("cars").delete().eq("id",id); } catch(e){ console.error(e); }
  ...
  const toggleAvailability=async id=>{
    ...
    try { await supabase.from("cars").update({available:nowAvailable}).eq("id",id); } catch(e){ console.error(e); }
    setUserCars(p=>p.map(c=>c.id===id?{...c,available:nowAvailable}:c));
  ...
  const sendRenewalRequest = async () => {
    try {
      await supabase.from("subscription_renewal_requests").insert({
        id: "ren"+Date.now(),
        agency_id: agency.id,
        requested_months: renewalMonths,
        monthly_price: 0,
        status: "pending",
    ...
    } catch(e){ console.error("Renewal error:",e); }
    setRenewalSent(true);
  ...
  const confirmBooking=async id=>{
    try { await supabase.from("bookings").update({status:"confirmed"}).eq("id",id); } catch(e){ console.error(e); }
  ...
  const cancelBooking=async id=>{
    try { await supabase.from("bookings").update({status:"cancelled"}).eq("id",id); } catch(e){ console.error(e); }
  ...
  const sendReply=async()=>{
    ...
    const newMsg={id:msgId,from:"agency",text:txt,time:new Date().toLocaleTimeString("fr-DZ",{hour:"2-digit",minute:"2-digit"}),read:true,status:"sent",...(attach?{attach}:{})};
    ...
    try {
      ...
      await supabase.from("messages").insert({
        ...
        user_name: conv?.user||"مستخدم",
        user_email: conv?.userEmail||null,
        subject: conv?.subject||"محادثة",
        body: txt,
        direction: "agency_to_user",
        status: "read",
      });
    } catch(e){ console.error("Reply save error:",e); }
    const mid=newMsg.id;
  ...
  // عند فتح محادثة - تحديث حالة الرسائل كـ "مقروءة"
  useEffect(()=>{
    if(!activeConv) return;
    const conv=convs.find(c=>c.id===activeConv);
    if(!conv||!conv.unread) return;
    setConvs(p=>p.map(c=>c.id===activeConv?{...c,unread:0}:c));
    if(supabaseReady && conv.userId){
      supabase.from("messages")
        .update({status:"read"})
        .eq("agency_id",agency?.id||"")
        .eq("user_id",conv.userId)
        .eq("direction","user_to_agency")
        .then(()=>{}).catch(()=>{});
    }
  },[activeConv]);
...
function groupMessages(rows){
  const by={};
  rows.forEach(r=>{
    // تجميع الرسائل حسب user_id (محادثة واحدة لكل مستخدم مع الوكالة)
    const k = r.user_id || r.id;
    if(!by[k]) by[k]={
      id:k,
      agencyId:r.agency_id,
      userId:r.user_id||null,
      user:r.user_name||r.user_email||"مستخدم",
      subject:r.subject||"محادثة",
      unread:0,
      msgs:[]
    };
    by[k].msgs.push({
      id:r.id,
      from:r.direction==="agency_to_user"?"agency":"user",
      text:r.body||r.subject||"رسالة",
      time:r.created_at||"",
      status:r.direction==="agency_to_user"?"sent":"received"
    });
    if(r.direction==="user_to_agency" && r.status==="pending") by[k].unread=(by[k].unread||0)+1;
  });
  return Object.values(by).sort((a,b)=>{
    const aLast=a.msgs[a.msgs.length-1]?.time||"";
    const bLast=b.msgs[b.msgs.length-1]?.time||"";
...
export default function App() {
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(async ({data:{session}})=>{
      ...
      try {
        ...
        if(!profile || !["agency","agency_owner"].includes(profile.role) || profile.status!=="active") { setSessionLoading(false); return; }
        ...
        const [bRes,mRes,cRes] = await Promise.all([
          supabase.from("bookings").select("*").eq("agency_id",profile.agency_id).order("created_at",{ascending:false}),
          supabase.from("messages").select("*").eq("agency_id",profile.agency_id).order("created_at",{ascending:true}),
          supabase.from("cars").select("*").eq("agency_id",profile.agency_id).order("created_at",{ascending:false}),
        ]);
        ...
        if(Array.isArray(cRes.data)) setUserCars(cRes.data.map(c=>({id:c.id,agencyId:c.agency_id,name:c.name||c.car_name||"سيارة",price:Number(c.price||0),image:c.img||c.image||"",status:c.status||"approved",available:c.available!==false,fuel:c.fuel||"",type:c.type||"sedan",year:c.year||new Date().getFullYear(),seats:c.seats||5,transmission:c.trans||"manual",badge:c.badge||"",wilaya:c.wilaya||"",img:c.img||c.image||""})));
        setLoggedIn(ag);
        // Realtime subscriptions after session restore
        supabase.channel("agency-rt-sess-"+profile.agency_id)
          .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`agency_id=eq.${profile.agency_id}`},(payload)=>{
            if(payload.new.direction==="user_to_agency"){
              setConvs(p=>{
                const existing=p.find(c=>c.userId===payload.new.user_id);
                const newMsg={id:payload.new.id,from:"user",text:payload.new.body,time:new Date().toLocaleTimeString("fr-DZ",{hour:"2-digit",minute:"2-digit"}),read:false,status:"received"};
                if(existing) return p.map(c=>c.userId===payload.new.user_id?{...c,msgs:[...c.msgs,newMsg],unread:(c.unread||0)+1}:c);
                return [{id:payload.new.id,userId:payload.new.user_id,user:payload.new.user_name||"مستخدم",subject:payload.new.subject||"رسالة جديدة",msgs:[newMsg],unread:1},...p];
              });
            }
          })
          .on("postgres_changes",{event:"INSERT",schema:"public",table:"bookings",filter:`agency_id=eq.${profile.agency_id}`},(payload)=>{
            setBookings(p=>[mapBookingRow(payload.new),...p]);
          })
          .on("postgres_changes",{event:"UPDATE",schema:"public",table:"bookings",filter:`agency_id=eq.${profile.agency_id}`},(payload)=>{
            setBookings(p=>p.map(b=>b.id===payload.new.id?mapBookingRow(payload.new):b));
          })
          .subscribe();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if(Array.isArray(bRes.data)) setBookings(bRes.data.map(mapBookingRow));
      if(Array.isArray(mRes.data)) setConvs(groupMessages(mRes.data));
      if(Array.isArray(cRes.data)) setUserCars(cRes.data.map(c=>({id:c.id,agencyId:c.agency_id,name:c.name||c.car_name||"سيارة",price:Number(c.price||0),image:c.img||c.image||"",status:c.status||"pending",available:c.available!==false,fuel:c.fuel||"",type:c.type||"sedan",year:c.year||new Date().getFullYear(),seats:c.seats||5,transmission:c.trans||"manual",badge:c.badge||"",wilaya:c.wilaya||"",img:c.img||c.image||""})));
      setLoggedIn(ag);
      setLoginErr("");
      // Realtime subscriptions
      supabase.channel("agency-rt-"+filter)
        .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`agency_id=eq.${filter}`},(payload)=>{
          if(payload.new.direction==="user_to_agency"){
            setConvs(p=>{
              const existing=p.find(c=>c.userId===payload.new.user_id);
              const newMsg={id:payload.new.id,from:"user",text:payload.new.body,time:new Date().toLocaleTimeString("fr-DZ",{hour:"2-digit",minute:"2-digit"}),read:false,status:"received"};
        ...
        .on("postgres_changes",{event:"INSERT",schema:"public",table:"bookings",filter:`agency_id=eq.${filter}`},(payload)=>{
          setBookings(p=>[mapBookingRow(payload.new),...p]);
        })
        .on("postgres_changes",{event:"UPDATE",schema:"public",table:"bookings",filter:`agency_id=eq.${filter}`},(payload)=>{
          setBookings(p=>p.map(b=>b.id===payload.new.id?mapBookingRow(payload.new):b));
        })
        .subscribe();
  ...
  if (sessionLoading) return (
    <div style={{position:"fixed",inset:0,background:"#07080F",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{color:"#7C3AED",fontSize:14,fontFamily:"'Cairo',sans-serif"}}>جاري التحميل...</div>
    </div>
  );
