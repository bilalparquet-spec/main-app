    ...
    <div style={{position:"fixed",inset:0,zIndex:700,background:"rgba(0,0,0,.72)",backdropFilter:"blur(10px)"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{padding:"18px 18px 14px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontWeight:900,fontSize:16,color:"#fff"}}>⚙️ الفلاتر</span>
          <button onClick={onClose} style={{background:"rgba(255,255,255,.08)",border:"none",color:"rgba(255,255,255,.6)",width:30,height:30,borderRadius:"50%",cursor:"pointer",fontSize:15,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:18,display:"flex",flexDirection:"column",gap:22}}>
          ...
          {/* Fuel */}
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>الوقود</div>
          ...
          {/* Trans */}
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>ناقل الحركة</div>
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
        ...
        <div style={{padding:18,borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",gap:10}}>
          <button onClick={()=>setAdvF({fuel:"الكل",trans:"الكل",verified:false,minP:"",maxP:""})} style={{flex:1,padding:"12px",borderRadius:11,border:"1px solid rgba(255,255,255,.1)",background:"rgba(255,255,255,.04)",color:"rgba(255,255,255,.5)",fontFamily:"inherit",fontWeight:700,fontSize:14,cursor:"pointer"}}>إعادة تعيين</button>
          <button onClick={()=>{setAdvF(L);onClose();}} style={{flex:2,padding:"12px",borderRadius:11,border:"none",background:"linear-gradient(135deg,#7C3AED,#4F46E5)",color:"#fff",fontFamily:"inherit",fontWeight:800,fontSize:14,cursor:"pointer",boxShadow:"0 4px 16px rgba(124,58,237,.38)"}}>تطبيق ✓</button>
        </div>
      </div>
    </div>
  );
