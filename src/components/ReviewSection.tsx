...
export function ReviewSection({ lang, targetType, targetId, reviews, setReviews, currentUser, openAuth }: Props) {
  ...
  const L: Record<string, any> = {
    ar: { title: "التقييمات والآراء", write: "اكتب تقييمك", ph: "شاركنا رأيك في هذه السيارة...", send: "إرسال التقييم", loginMsg: "سجّل دخولك لترك تقييم", loginBtn: "تسجيل الدخول", noReviews: "لا توجد تقييمات بعد. كن أول من يقيّم!", thankYou: "شكراً! تم إضافة تقييمك بنجاح 🎉", avg: "متوسط التقييم", total: "تقييم", yourRating: "تقييمك" },
    fr: { title: "Avis et évaluations", write: "Laisser un avis", ph: "Partagez votre expérience...", send: "Envoyer l'avis", loginMsg: "Connectez-vous pour laisser un avis", loginBtn: "Se connecter", noReviews: "Aucun avis encore. Soyez le premier!", thankYou: "Merci! Votre avis a été ajouté 🎉", avg: "Note moyenne", total: "avis", yourRating: "Votre note" },
    en: { title: "Reviews & Ratings", write: "Write a Review", ph: "Share your experience with this car...", send: "Submit Review", loginMsg: "Sign in to leave a review", loginBtn: "Sign In", noReviews: "No reviews yet. Be the first!", thankYou: "Thank you! Your review has been added 🎉", avg: "Average rating", total: "reviews", yourRating: "Your rating" },
  };
  const l = L[lang] || L.ar;
  ...
  const submit = () => {
    ...
    const nw: Review = {
      id: Date.now(), targetType, targetId, userId: currentUser.id,
      name: currentUser.name, phone: currentUser.phone, avatar: currentUser.avatar,
      rating: stars, comment: comment.trim(), date: new Date().toISOString().slice(0, 10),
    };
    setReviews(p => [nw, ...p]);
    ...
    <div style={{ marginTop: 36 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: "#fff", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {myReviews.length > 0 && (
            <span style={{ fontSize: 12, background: "rgba(124,58,237,.18)", border: "1px solid rgba(124,58,237,.3)", color: "#C084FC", padding: "2px 9px", borderRadius: 20, fontWeight: 600 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(245,158,11,.08)", border: "1px solid rgba(245,158,11,.22)", borderRadius: 11, padding: "6px 14px" }}>
            ...
            <div>
              <div style={{ display: "flex", gap: 2 }}>
                {[1,2,3,4,5].map(i => <span key={i} style={{ color: i <= Math.round(avg) ? "#F59E0B" : "rgba(255,255,255,.2)", fontSize: 13 }}>★</span>)}
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 5 }}>
                {[1,2,3,4,5].map(i => (
                  <span key={i}
                    onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(0)} onClick={() => setStars(i)}
                    style={{ fontSize: 28, cursor: "pointer", color: (hover || stars) >= i ? "#F59E0B" : "rgba(255,255,255,.18)", transition: "color .15s,transform .15s", display: "inline-block", transform: (hover || stars) >= i ? "scale(1.15)" : "scale(1)" }}>
                    ★
                  </span>
                ))}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 2, marginTop: 3 }}>
                  {[1,2,3,4,5].map(i => <span key={i} style={{ color: i <= r.rating ? "#F59E0B" : "rgba(255,255,255,.15)", fontSize: 11 }}>★</span>)}
                </div>
              </div>
            </div>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,.65)", lineHeight: 1.7 }}>{r.comment}</p>
          </div>
        ))}
      </div>
    </div>
