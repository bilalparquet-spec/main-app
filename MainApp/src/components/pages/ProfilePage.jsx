...
export function ProfilePage({ onLogout }) {
  ...
  // تاريخ الانضمام الحقيقي من created_at
  const joinedText = (() => {
    const raw = user?.created_at;
    if (!raw) return t.joined; // fallback للنص الثابت
    try {
      const locale = lang === "fr" ? "fr-DZ" : lang === "en" ? "en-US" : "ar-DZ";
      const formatted = new Intl.DateTimeFormat(locale, { month: "long", year: "numeric" }).format(new Date(raw));
      const prefix = { ar: "انضم في ", fr: "Inscrit en ", en: "Joined ", tz: "ⵉⵍⵍⴰ ⴷⵉ " }[lang] || "انضم في ";
    ...
    return (
      <div dir={dir} style={base}>
        ...
        <section style={{ textAlign: "center", padding: "28px 0 54px" }}>
          ...
          <p style={{ margin: 0, color: "rgba(255,255,255,.86)", fontSize: 16.4, fontWeight: 440 }}>{joinedText}</p>
        </section>
        <section>
