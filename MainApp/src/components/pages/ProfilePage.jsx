...
export function ProfilePage({ onLogout }) {
  ...
  if (pathname === "/profile/ask-ai") {
    return (
      <div dir={dir} style={base}>
        <TopBar title="Ask AI Drive Rent" dir={dir} onBack={() => navigate("/profile")} action={null} />
        <section style={{paddingTop:30}}>
          <div style={{background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.08)",borderRadius:16,padding:16}}>
            <h2 style={{fontSize:14,margin:"0 0 10px",fontWeight:900}}>Ask AI Drive Rent</h2>
            <p style={{fontSize:12,color:"rgba(255,255,255,.7)",lineHeight:1.6}}>Hello 👋 كيف يمكنني مساعدتك؟</p>
            <div style={{display:"grid",gap:10,marginTop:16}}>
              {["كيفية الحجز؟","طرق الدفع","التأمين","التواصل مع الوكالة"].map((q)=><button key={q} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,color:"#fff",padding:"12px",textAlign:"start",fontSize:12}}>{q}</button>)}
            </div>
          </div>
        </section>
      </div>
  ...
  if (pathname === "/profile/account/info") {
    return (
      <div dir={dir} style={base}>
        <TopBar title={t.accountInfo} dir={dir} onBack={() => navigate("/profile/account")} action={null} />
        <section style={{paddingTop:24}}>
          {[["Preferred first name","Ghouat"],["Email","bilalghouat5@gmail.com"],["Mobile phone","Not verified"],["Password","••••••••"],["Google","Not connected"]].map(([a,b]) => (
            <div key={a} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 0",borderBottom:"1px solid rgba(255,255,255,.08)"}}>
              <div>
                <div style={{fontSize:13,fontWeight:700}}>{a}</div>
                <div style={{fontSize:11,color:"rgba(255,255,255,.65)",marginTop:4}}>{b}</div>
              </div>
              <button style={{background:"transparent",border:"1px solid rgba(255,255,255,.12)",borderRadius:10,color:"#fff",padding:"8px 12px",fontSize:11}}>Edit</button>
            </div>
          ))}
        </section>
      </div>
  ...
  if (pathname === "/profile/account/notifications") {
    return (
      <div dir={dir} style={base}>
        <TopBar title={t.notificationSettings} dir={dir} onBack={() => navigate("/profile/account")} action={null} />
        <section style={{paddingTop:28}}>
          {["Trip and account updates (SMS)","Promotions and deals (SMS)","Push notifications","Promotions and announcements"].map((item)=> (
            <div key={item} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 0",borderBottom:"1px solid rgba(255,255,255,.08)"}}>
              <span style={{fontSize:12.5}}>{item}</span>
              <div style={{width:46,height:26,borderRadius:40,background:"#7C3AED"}} />
            </div>
          ))}
        </section>
      </div>
  ...
  return (
    <div dir={dir} className="compact-text" style={{ padding: "8px 18px 126px", animation: "fadeUp .28s ease", color: "#F7F7FA" }}>
      ...
      <section>
        ...
        <MoreRow dir={dir} icon={<IconSpark size={22} color="currentColor" strokeWidth={1.8} />} title="Ask AI Drive Rent" badge="New" onClick={() => navigate("/profile/ask-ai")} />
