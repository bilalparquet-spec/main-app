...
export function AgencyRegisterPage({ onBack } = {}) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // 1. إنشاء سجل في جدول agencies بحالة pending
      const agencyId = "ag" + Date.now();
      const agRes = await fetch(`${SUPABASE_URL}/rest/v1/agencies`, {
        method: "POST",
      ...
      if (!agRes.ok) {
        const agErr = await agRes.json().catch(() => ({}));
        throw new Error(agErr?.message || "فشل إنشاء سجل الوكالة");
      }

      // 2. إنشاء حساب Auth (الـ trigger سينشئ profile تلقائياً)
      const signUpRes = await fetch(`${SUPABASE_URL}/auth/v1/signup`, {
        ...
        body: JSON.stringify({
          ...
          data: { full_name: form.fullName, phone: form.phone, role: "agency_owner", agency_id: agencyId }
        }),
      });
