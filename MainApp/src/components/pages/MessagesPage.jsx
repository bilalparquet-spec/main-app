...
function useRealtimeMessages(agencyId, userId, onNewMessage) {
  useEffect(() => {
    if (!SUPABASE_READY || !agencyId || !userId) return;
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        ws.send(JSON.stringify({ topic: "realtime:public:messages", event: "phx_join", payload: { config: { broadcast: { ack: false }, presence: { key: "" }, postgres_changes: [{ event: "INSERT", schema: "public", table: "messages", filter: `agency_id=eq.${agencyId}` }] } }, ref: "1" }));
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (data.event === "postgres_changes" && data.payload?.data?.record) {
            const r = data.payload.data.record;
            if (r.user_id === userId || r.direction === "agency_to_user") onNewMessage(r);
          }
        } catch {}
      };
...
function ChatView({ conv, userId, onBack }) {
    ...
    const load = async () => {
      try {
        ...
        if (rows?.some(r => r.direction === "agency_to_user" && r.status !== "read")) {
          await dbUpdate("messages", `agency_id=eq.${conv.agencyId}&user_id=eq.${userId}&direction=eq.agency_to_user&status=neq.read`, { status: "read" }).catch(() => {});
        }
      } catch {}
    };
    load();
  ...
  // Realtime
  useRealtimeMessages(conv.agencyId, userId, (r) => {
    setMsgs(p => p.find(m => m.id === r.id) ? p : [...p, r]);
    if (r.direction === "agency_to_user") {
      dbUpdate("messages", `id=eq.${r.id}`, { status: "read" }).catch(() => {});
    }
  });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#050506", display: "flex", flexDirection: "column", zIndex: 100 }}>
      {/* Header */}
...
function ConvList({ convs, onOpen }) {
  if (!convs.length) return (
    <div style={{ paddingTop: 48, textAlign: "center" }}>
      ...
      <h2 style={{ margin: 0, color: "#fff", fontSize: 18, fontWeight: 950 }}>لا توجد رسائل</h2>
      <p style={{ margin: "8px 0 0", color: "rgba(255,255,255,.4)", fontSize: 13 }}>تواصل مع إحدى الوكالات</p>
    </div>
  );
...
export function MessagesPage() {
  ...
  // Global realtime for new messages (badge)
  useEffect(() => {
    if (!SUPABASE_READY || !user?.id) return;
    const wsUrl = SUPABASE_URL.replace("https://", "wss://").replace("http://", "ws://");
    let ws; let hb; let closed = false;
    const connect = () => {
      ws = new WebSocket(`${wsUrl}/realtime/v1/websocket?apikey=${SUPABASE_ANON_KEY}&vsn=1.0.0`);
      ws.onopen = () => {
        ws.send(JSON.stringify({ topic: "realtime:public:messages", event: "phx_join", payload: { config: { postgres_changes: [{ event: "INSERT", schema: "public", table: "messages" }] } }, ref: "1" }));
        hb = setInterval(() => ws.readyState === 1 && ws.send(JSON.stringify({ topic: "phoenix", event: "heartbeat", payload: {}, ref: "hb" })), 25000);
      };
      ws.onmessage = (e) => {
        try {
          const d = JSON.parse(e.data);
          if (d.event === "postgres_changes" && d.payload?.data?.record) {
            const r = d.payload.data.record;
      ...
      ws.onclose = () => { clearInterval(hb); if (!closed) setTimeout(connect, 3000); };
    };
    connect();
    return () => { closed = true; clearInterval(hb); ws?.close(); };
  }, [user?.id]);
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 170px)", padding: "4px 0 104px", animation: "fadeUp .28s ease", background: "#050506", color: "#fff" }}>
