      ...
      {tab === "notifications" ? (
        ...
        messages.length ? (
          <section style={{ padding: "14px 0 0", display: "grid", gap: 12 }}>
            {Object.values(messages.reduce((by, m) => {
              const k = m.agency_id || "x";
              if (!by[k]) by[k] = { agencyName: m.agency_name || t.agency, msgs: [] };
              by[k].msgs.push(m);
              return by;
            }, {})).map((conv, i) => (
              <article key={i} style={{ background: "rgba(255,255,255,.045)", border: "1px solid rgba(255,255,255,.09)", borderRadius: 14, padding: 11, textAlign: "start" }}>
                <strong style={{ color: "#fff", fontSize: 12.5, display: "block", marginBottom: 8 }}>🏢 {conv.agencyName}</strong>
                {conv.msgs.map((msg) => (
                  <div key={msg.id} style={{ display: "flex", justifyContent: msg.direction === "agency_to_user" ? "flex-start" : "flex-end", marginBottom: 6 }}>
                    <div style={{ maxWidth: "85%", background: msg.direction === "agency_to_user" ? "rgba(124,58,237,.15)" : "rgba(255,255,255,.08)", border: "1px solid " + (msg.direction === "agency_to_user" ? "rgba(124,58,237,.3)" : "rgba(255,255,255,.12)"), borderRadius: 10, padding: "7px 11px" }}>
                      <div style={{ fontSize: 10, color: msg.direction === "agency_to_user" ? "#A78BFA" : "rgba(255,255,255,.45)", marginBottom: 3 }}>{msg.direction === "agency_to_user" ? conv.agencyName : "أنت"}</div>
