...
export function TripsPage() {
  ...
  // ── Rate After Trip تلقائي ────────────────────────────────────────────────
  const { pending: ratingPending, dismiss: dismissRating } = useRateAfterTrip(allTrips);

  return (
    <div className="tiny-ui" style={{ paddingBottom: 100, animation: "fadeUp .35s ease" }}>
      {/* Rate After Trip Sheet */}
      {ratingPending && (
        <RateAfterTripSheet
          trip={ratingPending}
          onDone={dismissRating}
      ...
      {filtered.length === 0 && (
        <EmptyState variant="trips" onAction={() => { window.location.pathname = "/"; }} />
      )}
