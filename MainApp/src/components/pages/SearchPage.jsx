...
// ── Search History ────────────────────────────────────────────────────────────
const HISTORY_KEY = "driverent_search_history";
const MAX_HISTORY = 8;
...
// ── Virtual List wrapper للـ SearchPage ──────────────────────────────────────
const ITEM_HEIGHT = 96; // ارتفاع SearchCarRow بالـ px
const ITEM_GAP    = 10;

function VirtualSearchList({ results, onOpenCar, appLang }) {
  const { containerProps, items } = useVirtualList(results, {
  ...
  return (
    <div style={{ minHeight: "72vh", padding: "4px 0 108px", animation: "pageSlideIn .25s ease", color: "#F1F5F9" }}>
            ...
            {history.length > 0 && (
              <div style={{ marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,.35)", fontWeight: 700 }}>عمليات بحث سابقة</span>
                  <button
                    onClick={() => { clearHistory(); setHistory([]); }}
                    style={{ fontSize: 11, color: "rgba(255,255,255,.3)", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}
                  >مسح الكل</button>
                </div>
      ...
      {canShowResults && (results.length === 0 ? (
        ...
        <VirtualSearchList results={results} onOpenCar={onOpenCar} appLang={appLang} />
      ))}
    </div>
  );
}
