...
export function CarDetail({ car, onBack, liked, onLike, allReviews = [], addReview }) {
  ...
  // حساب الأيام والأسعار — يتحدث تلقائياً عند تغيير التواريخ
  const days = (() => {
    if (!pickupDate || !returnDate) return 1;
    const diff = Math.ceil((new Date(returnDate) - new Date(pickupDate)) / 86400000);
          ...
          <div style={bookingPageBodyStyle}>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ background: "rgba(255,255,255,.05)", borderRadius: 14, padding: "12px 14px" }}>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)", fontWeight: 700, marginBottom: 10 }}>عدد أيام الإيجار</div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <button
                    ...
                    style={dayStepBtnStyle}
                  >−</button>

                  <div style={{ textAlign: "center" }}>
                    <div style={{ color: "#A78BFA", fontSize: 34, fontWeight: 950, lineHeight: 1 }}>{days}</div>
                    <div style={{ color: "rgba(255,255,255,.5)", fontSize: 11.5, fontWeight: 700, marginTop: 3 }}>يوم</div>
                  </div>
              ...
              <div style={durationBadgeStyle}>
                <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,.45)", fontWeight: 700 }}>الاستلام</span>
                  <span style={{ fontSize: 12.5, color: "#fff", fontWeight: 800 }}>
                    {new Date(pickupDate).toLocaleDateString("ar-DZ", { weekday: "short", day: "numeric", month: "short" })}
                  </span>
                </div>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.3)" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                <div style={{ display: "flex", flexDirection: "column", gap: 3, textAlign: "right" }}>
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,.45)", fontWeight: 700 }}>الإرجاع</span>
                  <span style={{ fontSize: 12.5, color: "#fff", fontWeight: 800 }}>
                    {new Date(returnDate).toLocaleDateString("ar-DZ", { weekday: "short", day: "numeric", month: "short" })}
                  </span>
                </div>
              </div>
            </div>
