            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <label style={labelStyle}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!v) return;
                      setPickupDate(v);
                      // إذا تاريخ الإرجاع أصغر أو مساوٍ للاستلام — اضبطه تلقائياً
                      if (returnDate <= v) {
                ...
                <label style={labelStyle}>
                  ...
                  <input
                    ...
                    type="date"
                    min={pickupDate ? (() => { const d = new Date(pickupDate); d.setDate(d.getDate() + 1); return d.toISOString().slice(0, 10); })() : todayPlus(1)}
                    style={dateInputStyle}
                  />
                </label>
              </div>
              ...
              <div style={durationBadgeStyle}>
                <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,.5)", fontWeight: 700 }}>مدة الإيجار</span>
                  <span style={{ fontSize: 11.5, color: "rgba(255,255,255,.65)", fontWeight: 700 }}>
                    {pickupDate && returnDate
                      ? `${new Date(pickupDate).toLocaleDateString("ar-DZ", { day: "numeric", month: "short" })} ← ${new Date(returnDate).toLocaleDateString("ar-DZ", { day: "numeric", month: "short" })}`
                      : "حدد التواريخ"}
                  </span>
                </div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
                  <strong style={{ color: "#A78BFA", fontSize: 26, fontWeight: 950, lineHeight: 1 }}>{days}</strong>
                  <span style={{ color: "rgba(255,255,255,.6)", fontSize: 13, fontWeight: 800 }}>يوم</span>
                </div>
              </div>
            </div>
