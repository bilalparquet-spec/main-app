  ...
  return (
    ...
    >
      ...
      <div style={bottomBarStyle}>
        ...
        <button onClick={() => { if (!getCurrentUser()) setGuestAuthSheet(true); else { setBookingStep("dates"); setBookingPage(true); } }} style={bookButtonStyle}>حجز الآن</button>
      </div>
      ...
      {bookingPage && (
        <div style={bookingPageStyle}>
          ...
          {/* ── Header مع زر الرجوع وعنوان الخطوة ── */}
          <header style={bookingPageHeaderStyle}>
            <button
              type="button"
              onClick={() => {
                if (bookingStep === "dates")  setBookingPage(false);
                if (bookingStep === "driver") setBookingStep("dates");
                if (bookingStep === "review") setBookingStep("driver");
              }}
              aria-label="رجوع"
            ...
            <strong style={{ fontSize: 16, fontWeight: 950 }}>
              {bookingStep === "dates"  && "التاريخ والموقع"}
              {bookingStep === "driver" && "بيانات السائق"}
              {bookingStep === "review" && "مراجعة وتأكيد"}
            </strong>
            <span style={{ width: 42 }} />
          </header>
          ...
          <div style={{
            ...
            {[
              { key: "dates",  label: "التاريخ",  num: 1 },
              { key: "driver", label: "السائق",   num: 2 },
              { key: "review", label: "التأكيد",  num: 3 },
            ].map((s, i, arr) => {
              const done    = (bookingStep === "driver" && s.key === "dates")
                           || (bookingStep === "review" && s.key !== "review");
              const active  = bookingStep === s.key;
              return (
                <div key={s.key} style={{ display: "flex", alignItems: "center" }}>
                  ...
                  {i > 0 && (
                    <div style={{
                      ...
                      height: 2,
                      borderRadius: 2,
                      background: done || active ? "#7C3AED" : "rgba(255,255,255,.12)",
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      height: 30,
                      borderRadius: "50%",
                      background: done ? "#7C3AED" : active ? "rgba(124,58,237,.25)" : "rgba(255,255,255,.06)",
                      border: `2px solid ${done || active ? "#7C3AED" : "rgba(255,255,255,.15)"}`,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      ...
                      {done ? (
                        <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                          <path d="M2.5 6.5L5.5 9.5L10.5 3.5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      ) : (
                        <span style={{
                          fontSize: 11,
                    ...
                    <span style={{
                      fontSize: 10,
                      fontWeight: 800,
                      color: active ? "#A78BFA" : done ? "rgba(167,139,250,.7)" : "rgba(255,255,255,.35)",
          ...
          {bookingStep === "dates" && (
            <>
              <div style={bookingPageBodyStyle}>
                ...
                {/* اختيار التاريخ */}
                <div style={{ ...smallCardStyle, padding: 16, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 14px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M7 3v4"/><path d="M17 3v4"/><path d="M4 8h16"/><path d="M5 5h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Z"/></svg>تاريخ الرحلة</span></h3>
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: 12, color: "rgba(255,255,255,.6)", fontWeight: 800 }}>مدة الإيجار</span>
                    <strong style={{ color: "#A78BFA", fontSize: 13.5 }}>{days} يوم</strong>
                  </div>
                </div>

                {/* الموقع */}
                <div style={{ ...smallCardStyle, padding: 16, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 12px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s7-6.2 7-12a7 7 0 1 0-14 0c0 5.8 7 12 7 12Z"/><path d="M12 12a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"/></svg>موقع الاستلام</span></h3>
                  <div style={{ display: "grid", gridTemplateColumns: "22px 1fr", alignItems: "center", gap: 9 }}>
                    <IconPin size={16} color="rgba(255,255,255,.68)" />
                    <span style={{ color: "rgba(255,255,255,.75)", fontSize: 12.4, fontWeight: 800 }}>{agency.name} · {car.wilaya}</span>
                  </div>
                </div>

                {/* ملخص السعر */}
                <div style={{ ...smallCardStyle, padding: 14, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 12px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 5h14"/><path d="M5 19h14"/><path d="M7 5v14"/><path d="M17 5v14"/><path d="M9 9h6a3 3 0 0 1 0 6H9V9Z"/></svg>ملخص السعر</span></h3>
                  <div style={{ ...priceRowStyle, marginBottom: 9 }}><span>تسعيرة السيارة</span><strong>{formatDzd(baseTotal)}</strong></div>
                  <div style={{ ...priceRowStyle, marginBottom: 9 }}><span>رسوم المنصة 5%</span><strong>{formatDzd(platformFee)}</strong></div>
                  <div style={{ ...priceRowStyle, color: "#fff", fontSize: 13.8, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,.08)" }}>
                    <span>السعر الإجمالي</span><strong>{formatDzd(total)}</strong>
                  </div>
                </div>
              ...
              <footer style={bookingPageFooterStyle}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ color: "rgba(255,255,255,.42)", fontWeight: 750, fontSize: 10.6 }}>{days} يوم · قبل الضرائب</div>
                </div>
                <button type="button" onClick={() => setBookingStep("driver")} style={bookButtonStyle}>التالي ←</button>
              </footer>
            </>
          )}

          {/* ════════════════════════════════════════════
              الخطوة 2: بيانات السائق
          ════════════════════════════════════════════ */}
          {bookingStep === "driver" && (
            <>
              <div style={bookingPageBodyStyle}>

                {/* ملخص التاريخ المختار */}
                <div style={{
                  display: "flex",
                  gap: 8,
                  padding: "10px 14px",
                  ...
                  <span style={{display:"inline-flex",alignItems:"center"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M7 3v4"/><path d="M17 3v4"/><path d="M4 8h16"/><path d="M5 5h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Z"/></svg></span>
                  <span style={{ fontSize: 11.8, color: "rgba(255,255,255,.72)", fontWeight: 800 }}>
                    {formatArabicDate(pickupDate)} ← {formatArabicDate(returnDate)} · {days} أيام
                  </span>
                </div>

                {/* بيانات السائق */}
                <div style={{ ...smallCardStyle, padding: 16, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 16px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 5h18v14H3V5Z"/><path d="M7 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z"/><path d="M5 15c1-2 3-2 4-2s3 0 4 2"/><path d="M15 9h4"/><path d="M15 13h4"/></svg>بيانات السائق</span></h3>
                  ...
                  <label style={{ ...labelStyle, marginBottom: 12 }}>
                    الاسم الأول (كما في رخصة القيادة)
                  ...
                  <label style={labelStyle}>
                    ...
                    <select value={driverAge} onChange={(e) => setDriverAge(e.target.value)} style={fieldStyle}>
                      {Array.from({ length: 53 }, (_, i) => String(i + 18)).map((age) => <option key={age} value={age}>{age} سنة</option>)}
                    </select>
                  </label>
                  <p style={{ margin: "12px 0 0", color: "rgba(255,255,255,.42)", fontSize: 11, lineHeight: 1.7, fontWeight: 650 }}>
                    بإدخال رقم الهاتف توافق على استلام رسائل مرتبطة بالحجز.
                  </p>
                </div>
              </div>
              ...
              <footer style={bookingPageFooterStyle}>
                <div style={{ minWidth: 0 }}>
                  <strong style={realPriceStyle}>{formatDzd(total)}</strong>
                  <div style={{ color: "rgba(255,255,255,.42)", fontWeight: 750, fontSize: 10.6 }}>الإجمالي شامل الرسوم</div>
                </div>
                <button type="button" onClick={() => setBookingStep("review")} style={bookButtonStyle}>مراجعة ←</button>
              </footer>
            </>
          )}
          ...
          {bookingStep === "review" && (
            <>
              <div style={bookingPageBodyStyle}>
                ...
                {/* تفاصيل الرحلة */}
                <div style={{ ...smallCardStyle, padding: 14, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 12px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 4h12a2 2 0 0 1 2 2v14l-4-2-4 2-4-2-4 2V6a2 2 0 0 1 2-2Z"/><path d="M8 9h8"/><path d="M8 13h6"/></svg>ملخص الحجز</span></h3>
                  ...
                  <ReviewRow label="المدة"           value={`${days} يوم`} />
                  <ReviewRow label="الموقع"          value={`${agency.name} · ${car.wilaya}`} />
                </div>

                {/* بيانات السائق */}
                <div style={{ ...smallCardStyle, padding: 14, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 12px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 5h18v14H3V5Z"/><path d="M7 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z"/><path d="M5 15c1-2 3-2 4-2s3 0 4 2"/><path d="M15 9h4"/><path d="M15 13h4"/></svg>بيانات السائق</span></h3>
                  <ReviewRow label="الاسم"      value={fullDriverName} />
                  <ReviewRow label="رقم الهاتف" value={fullDriverPhone || "غير محدد"} />
                  <ReviewRow label="العمر"      value={`${driverAge} سنة`} />
                </div>

                {/* ملخص السعر النهائي */}
                <div style={{ ...smallCardStyle, padding: 14, background: "rgba(255,255,255,.045)" }}>
                  <h3 style={{ margin: "0 0 12px", color: "#fff", fontSize: 14.5, fontWeight: 950 }}><span style={{display:"inline-flex",alignItems:"center",gap:7}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 5h14"/><path d="M5 19h14"/><path d="M7 5v14"/><path d="M17 5v14"/><path d="M9 9h6a3 3 0 0 1 0 6H9V9Z"/></svg>السعر النهائي</span></h3>
                  <div style={{ ...priceRowStyle, marginBottom: 9 }}><span>تسعيرة السيارة ({days} يوم)</span><strong>{formatDzd(baseTotal)}</strong></div>
                  <div style={{ ...priceRowStyle, marginBottom: 9 }}><span>رسوم المنصة 5%</span><strong>{formatDzd(platformFee)}</strong></div>
                  <div style={{ ...priceRowStyle, color: "#fff", fontSize: 14, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,.08)" }}>
                    <span>الإجمالي</span>
                    <strong style={{ color: "#A78BFA", fontSize: 15 }}>{formatDzd(total)}</strong>
                  </div>
                </div>
              ...
              <footer style={bookingPageFooterStyle}>
                <div style={{ minWidth: 0 }}>
                  <strong style={{ ...realPriceStyle, color: "#A78BFA" }}>{formatDzd(total)}</strong>
