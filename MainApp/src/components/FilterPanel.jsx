...
const COPY = {
  ...
  tz: { title: "ⵉⴼⵉⵍⵜⵔⵏ", price: "ⴰⵜⵉⴳ ⵏ ⵓⵙⵙ", from: "ⵙⴳ", to: "ⴰⵔ", fuel: "ⴰⵙⵓⴷⴷⵓ", transmission: "ⴰⵏⴰⵇⵇⴰⵍ", verified: "ⵜⵉⵡⴽⵍⴰⵜ ⵉⵎⵓⵜⵜⵇⴰⵏ", reset: "ⴰⵍⵍⵙ", apply: "ⵙⵙⵏⵜⵉ", fuels: { all: "ⴰⴽⴽ", gas: "ⴱⵏⵣⵉⵏ", diesel: "ⴷⵉⵣⵉⵍ", electric: "ⴽⴰⵀⵔⴰⴱⴰ" }, transmissions: { all: "ⴰⴽⴽ", auto: "ⴰⵡⵜⵓⵎⴰⵜⵉⴽ", manual: "ⵎⴰⵏⵓⵡⴰⵍ" } },
};
...
export function FilterPanel({advF, setAdvF, onClose}) {
  ...
  const transOptions = [
    { value: "ALL", label: t.transmissions.all },
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{padding:"18px 18px 14px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <span style={{fontWeight:900,fontSize:16,color:"#fff"}}>{t.title}</span>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:18,display:"flex",flexDirection:"column",gap:22}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>{t.price}</div>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              {fuelOptions.map(f=>(
                <button key={f.value} onClick={()=>up("fuel",f.value)} style={{
                  padding:"7px 14px",borderRadius:40,cursor:"pointer",fontFamily:"inherit",fontWeight:700,fontSize:12,
                  border:`1.5px solid ${L.fuel===f.value?"#7C3AED":"rgba(255,255,255,.1)"}`,
                  background:L.fuel===f.value?"rgba(124,58,237,.2)":"rgba(255,255,255,.04)",
                  color:L.fuel===f.value?"#C084FC":"rgba(255,255,255,.5)",transition:"all .2s",
                }}>{f.label}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              {transOptions.map(tr=>(
                <button key={tr.value} onClick={()=>up("trans",tr.value)} style={{
                  padding:"7px 14px",borderRadius:40,cursor:"pointer",fontFamily:"inherit",fontWeight:700,fontSize:12,
                  border:`1.5px solid ${L.trans===tr.value?"#7C3AED":"rgba(255,255,255,.1)"}`,
                  background:L.trans===tr.value?"rgba(124,58,237,.2)":"rgba(255,255,255,.04)",
                  color:L.trans===tr.value?"#C084FC":"rgba(255,255,255,.5)",transition:"all .2s",
                }}>{tr.label}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(255,255,255,.04)",borderRadius:13,padding:"13px 15px"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:13,color:"rgba(255,255,255,.78)",fontWeight:600}}>{t.verified}</span>
            </div>
        ...
        <div style={{padding:18,borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",gap:10}}>
          <button onClick={()=>setAdvF({fuel:"ALL",trans:"ALL",verified:false,minP:"",maxP:""})}
