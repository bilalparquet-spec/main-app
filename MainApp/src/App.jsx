    ...
    if (pathname.startsWith("/car/") && selCar)
      return (
        <ErrorBoundary page="تفاصيل السيارة" onBack={()=>{ setSelCar(null); navigate(-1); }}>
          <CarDetail car={selCar} onBack={()=>{ setSelCar(null); navigate(-1); }} liked={liked.has(String(selCar.id))} onLike={()=>toggleLike(selCar.id)} allReviews={reviews} addReview={r=>setReviews(p=>[r,...p])}/>
        </ErrorBoundary>
      );
