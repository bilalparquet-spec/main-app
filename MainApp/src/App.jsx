...
export default function App() {
  ...
  }[appLang] || { explore:"ⴰⵙⵏⵓⴱⴳ", searchTab:"ⴰⵔⵣⵣⵓ", favorites:"ⵉⵎⴰⵖⵏⴰⵙⵏ", trips:"ⵜⵉⵔⵣⴰ", messages:"ⵉⵣⵏⴰⵏ", profile:"ⵓⴳⴳⴰⵔ", loginRequired:"ⵙⵙⵉⴷⴼ ⴰⴽⴽⵏ ⴰⵜⴽⵎⵎⵍⴷ", loginSub:"", login:"ⵙⵙⵉⴷⴼ", register:"ⵙⵏⵓⵍⴼⵓ ⴰⵎⵉⴹⴰⵏ", nearby:"ⵇⵔⵉⴱ", suggest:"ⵙⵓⵎⵔ ⴰⵙⵉⴷ", search:"ⵔⵣⵓ: ⴰⵙⵉⴷ, ⵜⴰⵡⵉⵍⴰⵢⵜ..." };
  ...
  // Guests must not enter Favorites: keep current page/home and show auth bottom sheet.
  useEffect(() => {
  ...
  const renderPage = () => {
    ...
    if ((["/trips", "/messages"].includes(pathname) || pathname.startsWith("/profile")) && !isLoggedIn) {
      return (
        <div style={{minHeight:"62vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"34px 16px",animation:"fadeUp .3s ease"}}>
          <div style={{width:"100%",maxWidth:400,textAlign:"center"}}>
            <div style={{width:72,height:72,borderRadius:24,margin:"0 auto 20px",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,rgba(124,58,237,.18),rgba(79,70,229,.1))",border:"1px solid rgba(124,58,237,.28)",fontSize:32,boxShadow:"0 8px 32px rgba(124,58,237,.2)"}}>🔐</div>
            <h2 style={{margin:"0 0 10px",fontSize:22,fontWeight:950,color:"#fff",letterSpacing:"-.5px"}}>{tx.loginRequired}</h2>
            <p style={{margin:"0 0 24px",fontSize:13.5,color:"rgba(255,255,255,.44)",lineHeight:1.8}}>{tx.loginSub}</p>
            <div style={{display:"flex",flexDirection:"column",gap:11}}>
              <button onClick={()=>navigate("/login")} style={{width:"100%",border:"none",borderRadius:16,padding:"15px 16px",background:"linear-gradient(135deg,#7C3AED,#4F46E5)",color:"#fff",fontFamily:"inherit",fontSize:15,fontWeight:900,cursor:"pointer",boxShadow:"0 6px 22px rgba(124,58,237,.38)"}}>{tx.login}</button>
              <button onClick={()=>navigate("/register")} style={{width:"100%",border:"1.5px solid rgba(167,139,250,.3)",borderRadius:16,padding:"14px 16px",background:"rgba(124,58,237,.08)",color:"#C4B5FD",fontFamily:"inherit",fontSize:14,fontWeight:900,cursor:"pointer"}}>{tx.register}</button>
            </div>
          </div>
        </div>
      );
    ...
    if (pathname === "/favorites")
      return <FavoritesPage liked={liked} cars={sbCars} onLike={toggleLike} onOpenCar={openCar} isLoggedIn={isLoggedIn} appLang={appLang} onRequireAuth={()=>setAuthSheet(true)} />;
    ...
    return (
      <>
        {/* ── Hero ── */}
        <div style={{textAlign:"center",padding:"var(--hero-padding, 26px 12px 20px)",animation:"fadeUp .5s ease",position:"relative"}}>
          ...
          {userWilaya ? (
            <div
              ...
              style={{display:"inline-flex",alignItems:"center",gap:6,marginBottom:12,padding:"5px 12px 5px 10px",borderRadius:999,background:"rgba(52,211,153,.1)",border:"1px solid rgba(52,211,153,.28)",color:"#34D399",fontSize:11.5,fontWeight:800,cursor:"pointer",boxShadow:"0 2px 12px rgba(52,211,153,.15)"}}>
              <IconPin size={12} color="#34D399"/>
              {userWilaya}
              <IconClose size={10} color="#34D399"/>
            </div>
          ) : (
            <div style={{display:"inline-flex",alignItems:"center",gap:6,marginBottom:12,padding:"4px 14px",borderRadius:999,background:"rgba(124,58,237,.12)",border:"1px solid rgba(124,58,237,.24)",color:"#C4B5FD",fontSize:11,fontWeight:800,letterSpacing:".3px"}}>
              <div style={{width:6,height:6,borderRadius:"50%",background:"#A78BFA",boxShadow:"0 0 6px #A78BFA"}}/>
              أكثر من +1000 سيارة في الجزائر
            </div>
          )}

          {/* H1 */}
          <h1 style={{
            fontSize:"clamp(24px,5.5vw,40px)", fontWeight:950, lineHeight:1.15,
            background:"linear-gradient(135deg, #F1F5F9 30%, #A78BFA 70%, #818CF8)",
            WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent",
            margin:"0 0 22px", letterSpacing:"-1.5px",
            textShadow:"none",
          }}>
            {userWilaya ? `سيارات في ${userWilaya}` : "أجر سيارتك المثالية\nبكل سهولة"}
          </h1>

          {/* Search bar */}
          <div style={{maxWidth:"clamp(300px,92vw,540px)",margin:"0 auto",position:"relative"}}>
            <div style={{display:"flex",gap:10}}>
              <div style={{flex:1,position:"relative"}}>
                <div style={{
                  display:"flex",alignItems:"center",
                  background:"rgba(255,255,255,.065)",
                  border:`1.5px solid ${searchFocus?"rgba(124,58,237,.7)":"rgba(255,255,255,.12)"}`,
                  borderRadius:15,
                  transition:"border-color .22s, box-shadow .22s",
                  boxShadow:searchFocus?"0 0 0 4px rgba(124,58,237,.14)":"0 4px 20px rgba(0,0,0,.18)",
                }}>
                  <span style={{padding:"0 12px",color:searchFocus?"#A78BFA":"rgba(255,255,255,.28)",flexShrink:0,display:"flex",alignItems:"center"}}>
                    <IconSearchSm size={16} color={searchFocus?"#A78BFA":"rgba(255,255,255,.28)"}/>
                  </span>
                  <input
                    ref={searchRef}
                    value={searchText}
                    ...
                    style={{flex:1,background:"transparent",border:"none",color:"#fff",padding:"14px 6px",fontSize:14.5,fontFamily:"inherit",outline:"none",minWidth:0,fontWeight:600}}
                  />
                  {searchText&&(
                    <button onClick={()=>setSearchText("")} style={{background:"none",border:"none",color:"rgba(255,255,255,.38)",cursor:"pointer",padding:"0 12px",display:"flex",alignItems:"center",minHeight:46}}>
                      <IconClose size={15} color="rgba(255,255,255,.38)"/>
                    </button>
                  )}
                </div>
                ...
                {searchFocus&&suggs.length>0&&(
                  <div style={{position:"absolute",top:"calc(100% + 8px)",left:0,right:0,zIndex:500,background:"#0C0D1E",border:"1px solid rgba(124,58,237,.3)",borderRadius:15,overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,.65)",animation:"fadeUp .18s ease"}}>
                    {suggs.map((c,i)=>(
                      <div key={c.id} className="sug" onMouseDown={()=>openCar(c)} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 14px",cursor:"pointer",borderBottom:i<suggs.length-1?"1px solid rgba(255,255,255,.055)":"none"}}>
                        <img src={c.img} alt="" style={{width:46,height:34,objectFit:"cover",borderRadius:9,flexShrink:0}}/>
                        <div style={{flex:1,minWidth:0}}>
                          <div data-no-i18n="true" style={{fontWeight:800,fontSize:13.5,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</div>
                          <div style={{fontSize:11,color:"rgba(255,255,255,.36)",marginTop:2}}>{c.wilaya} · {c.price.toLocaleString()} دج/يوم</div>
                        </div>
              ...
              <button
                ...
                style={{
                  ...
                  background:nActive>0?"linear-gradient(135deg,#7C3AED,#4F46E5)":"rgba(255,255,255,.07)",
                  border:`1.5px solid ${nActive>0?"transparent":"rgba(255,255,255,.12)"}`,
                  color:nActive>0?"#fff":"rgba(255,255,255,.55)",
                  padding:"0 15px",borderRadius:15,cursor:"pointer",fontFamily:"inherit",
                  fontWeight:800,fontSize:13,flexShrink:0,transition:"all .22s",
                  minHeight:48,boxShadow:nActive>0?"0 4px 18px rgba(124,58,237,.38)":"none",
                }}>
                <IconFilter size={14} color={nActive>0?"#fff":"rgba(255,255,255,.55)"}/>
                فلتر {nActive>0&&<span style={{background:"rgba(255,255,255,.25)",borderRadius:999,padding:"0 5px",fontSize:10.5,fontWeight:900,marginRight:2}}>{nActive}</span>}
              </button>
            </div>

            {/* Quick action pills */}
            <div style={{display:"flex",gap:8,overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",padding:"13px 2px 0",justifyContent:"center",flexWrap:"wrap"}}>
              <button onClick={refreshLocation} className="btn-press" style={{flexShrink:0,display:"inline-flex",alignItems:"center",gap:6,border:"1px solid rgba(124,58,237,.28)",background:"rgba(124,58,237,.1)",color:"#C4B5FD",borderRadius:999,padding:"9px 14px",fontFamily:"inherit",fontSize:11.5,fontWeight:800,cursor:"pointer",boxShadow:"0 2px 10px rgba(124,58,237,.15)"}}>
                <IconMap size={13} color="#A78BFA"/>
                {locStatus==="requesting"?"تحديد الموقع...":tx.nearby}
              </button>
              <button onClick={()=>setSmartOpen(true)} className="btn-press" style={{flexShrink:0,display:"inline-flex",alignItems:"center",gap:6,border:"1px solid rgba(124,58,237,.28)",background:"linear-gradient(135deg,rgba(124,58,237,.2),rgba(79,70,229,.12))",color:"#fff",borderRadius:999,padding:"9px 14px",fontFamily:"inherit",fontSize:11.5,fontWeight:800,cursor:"pointer",boxShadow:"0 2px 10px rgba(124,58,237,.2)"}}>
  ...
  return (
    <div dir={appDir} data-app-lang={appLang} style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      {/* ── Top Nav ─────────────────────────────────────────────────────── */}
      {showTopNav && <nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(6,7,17,.95)",backdropFilter:"blur(22px)",WebkitBackdropFilter:"blur(22px)",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"var(--standalone-extra-top, 0px) clamp(14px,3vw,24px) 0",height:"var(--top-nav-total, var(--nav-height, 62px))",display:"flex",alignItems:"center",gap:"clamp(8px,2vw,16px)",boxShadow:"0 1px 0 rgba(255,255,255,.05), 0 4px 20px rgba(0,0,0,.3)"}}>
        ...
        <button onClick={()=>navigate("/agency-register")} style={{
          ...
          gap:3, background:"linear-gradient(135deg,#7C3AED,#4F46E5)",
          border:"none", borderRadius:13, padding:"8px clamp(10px,2.5vw,14px)",
          cursor:"pointer", fontFamily:"inherit", flexShrink:0, transition:"all .18s",
          minWidth:"clamp(54px,14vw,70px)", boxShadow:"0 4px 16px rgba(124,58,237,.35)",
        }}
          ...
          <span style={{fontSize:"clamp(9px,2.2vw,11px)", fontWeight:900, color:"#fff", whiteSpace:"nowrap", lineHeight:1.4, textAlign:"center"}}>أضف<br/>وكالتك</span>
        </button>
      </nav>}
      ...
      {/* ── Bottom Nav ───────────────────────────────────────────────────── */}
      {!isFullDetailRoute && <div className="bottom-nav" dir={appDir} data-nav-dir={appDir} style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(6,7,17,.97)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",flexDirection:"row",direction:appDir,justifyContent:"space-around",padding:"8px 0 max(10px,env(safe-area-inset-bottom))",zIndex:99}}>
        {[
          ...
          const isHome = path === "/";
          const active = isHome
            ? (pathname === "/" || pathname.startsWith("/car/") || pathname.startsWith("/agency/") || pathname.startsWith("/all/"))
            : pathname === path;
          const isProtected = !isHome;
          return (
            <button key={label} onClick={()=>{
              if (isProtected && !isLoggedIn) { setAuthSheet(true); return; }
              navigate(path); window.scrollTo(0,0);
            }} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",padding:"4px clamp(6px,2vw,14px)",transition:"opacity .2s",flex:1,maxWidth:80,position:"relative"}}>
              {active && <div style={{position:"absolute",top:-8,left:"50%",transform:"translateX(-50%)",width:28,height:3,borderRadius:"0 0 3px 3px",background:"linear-gradient(90deg,#7C3AED,#818CF8)",boxShadow:"0 2px 8px rgba(124,58,237,.5)"}}/>}
              <Icon active={active}/>
              <span className="nav-label" style={{fontSize:"clamp(9px,2.5vw,11px)",fontWeight:active?800:500,color:active?"#A78BFA":"rgba(255,255,255,.32)",transition:"color .2s"}}>{label}</span>
            </button>
          );
        })}
      </div>}
