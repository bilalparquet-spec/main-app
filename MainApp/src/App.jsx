...
import { useSwipeBack, SwipeIndicator } from "./hooks/useSwipeBack.jsx";
...
import { useOnlineStatus, useOfflineCache } from "./hooks/useOfflineCache.js";
...
export default function App() {
  ...
  const [fType, setFTypeRaw] = useState(() => {
    try { return localStorage.getItem("driverent_ftype") || "all"; } catch { return "all"; }
  });
  const setFType = (v) => { setFTypeRaw(v); try { localStorage.setItem("driverent_ftype", v); } catch {} };

  const [advF, setAdvFRaw] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("driverent_advf") || "null");
      return saved || { fuel:"ALL", trans:"ALL", verified:false, minP:"", maxP:"" };
    } catch { return { fuel:"ALL", trans:"ALL", verified:false, minP:"", maxP:"" }; }
  });
  const setAdvF = (v) => {
    const next = typeof v === "function" ? v(advF) : v;
    setAdvFRaw(next);
    try { localStorage.setItem("driverent_advf", JSON.stringify(next)); } catch {}
  };

  const [searchText, setSearchText] = useState("");
  const [searchFocus, setSearchFocus] = useState(false);
  const { liked, toggleLike: toggleLikeDB } = useFavorites();
  const [sort, setSort]       = useState("pop");
  // reviews تُدار الآن داخل CarDetail مباشرة (تُجلب من Supabase عند فتح السيارة)
  const { cars: sbCars, loading: carsLoading, refetch: refetchCars } = useCars();
  ...
  // ── Booking Reminder — إشعار تلقائي قبل يوم من الرحلة ────────────────────
  useBookingReminder();
  ...
  useEffect(() => {
    ...
    if (type === "agency") {
      const cached = offlineCache.getAgencies();
      const found  = cached.find(a => String(a.id) === id);
      if (found) { setSelAgency(found); return; }
      import("./lib/supabase.js").then(({ dbSelect }) =>
        dbSelect("agencies", `?id=eq.${id}&limit=1`)
          .then(rows => { if (rows?.[0]) setSelAgency(rows[0]); })
          .catch(() => null)
      );
    }
  }, [pathname, sbCars]); // eslint-disable-line
  ...
  // طلب إذن الإشعارات عند أول تسجيل دخول
  useEffect(() => {
    if (isLoggedIn && notif.permission === "default") {
      // تأخير بسيط لعدم مقاطعة تجربة الدخول
      const t = setTimeout(() => notif.requestPermission(), 3500);
      return () => clearTimeout(t);
    }
  }, [isLoggedIn]); // eslint-disable-line

  // ── Swipe Back Gesture (محسّن مع مؤشر بصري) ──────────────────────────────
  const { swipeProgress } = useSwipeBack({
    pathname,
    navigate,
    onVibrate: haptic.swipeBack,
  });
  ...
  const toggleLike = async (id) => {
    ...
    await toggleLikeDB(id);
    if (wasLiked) { toast.info("تمت الإزالة من المفضلة"); }
    else          { toast.success("تمت الإضافة للمفضلة ❤️"); }
  };
  ...
  const filtered = sbCars
    ...
    .filter(c=>{ if(!debouncedSearch) return true; const q=debouncedSearch.toLowerCase(); return c.name.toLowerCase().includes(q)||c.badge.includes(debouncedSearch)||c.wilaya.includes(debouncedSearch); })
    .filter(c=>advF.fuel==="ALL"||c.fuel===advF.fuel)
    .filter(c=>advF.trans==="ALL"||c.trans===advF.trans)
  ...
  const nActive = [advF.fuel!=="ALL",advF.trans!=="ALL",advF.verified,advF.minP,advF.maxP].filter(Boolean).length;
  ...
  const renderPage = () => {
    if (pathname.startsWith("/car/") && selCar)
      return (
        <ErrorBoundary page="تفاصيل السيارة" onBack={()=>{ setSelCar(null); navigate(-1); }}>
          <CarDetail car={selCar} onBack={()=>{ setSelCar(null); navigate(-1); }} liked={liked.has(String(selCar.id))} onLike={()=>toggleLike(selCar.id)}/>
        </ErrorBoundary>
      );

    if (pathname.startsWith("/agency/") && selAgency)
      return (
        <ErrorBoundary page="صفحة الوكالة" onBack={()=>{ setSelAgency(null); navigate(-1); }}>
          <AgencyDetail agency={selAgency} cars={sbCars} onBack={()=>{ setSelAgency(null); navigate(-1); }} onOpenCar={openCar} liked={liked} onLike={toggleLike}/>
        </ErrorBoundary>
      );

    if (pathname === "/search")
      return (
        <ErrorBoundary page="البحث" onBack={()=>navigate("/")}>
          <SearchPage cars={sbCars} onBack={()=>navigate("/")} onOpenCar={openCar} appLang={appLang} userWilaya={userWilaya}/>
        </ErrorBoundary>
      );
    ...
    if (pathname.startsWith("/all/")) {
      ...
      return (
        <ErrorBoundary page="عرض القسم" onBack={()=>navigate("/")}>
          <AllSectionPage
            section={section}
    ...
    if (pathname === "/favorites")
      return (
        <ErrorBoundary page="المفضلة" onBack={()=>navigate("/")}>
          <FavoritesPage liked={liked} cars={sbCars} onLike={toggleLike} onOpenCar={openCar} isLoggedIn={isLoggedIn} appLang={appLang} onRequireAuth={()=>setAuthSheet(true)} />
        </ErrorBoundary>
      );

    if (pathname === "/trips")
      return (
        <ErrorBoundary page="رحلاتي" onBack={()=>navigate("/")}>
          <TripsPage/>
        </ErrorBoundary>
      );
    ...
    if (pathname === "/messages")
      return (
        <ErrorBoundary page="الرسائل" onBack={()=>navigate("/")}>
          <MessagesPage/>
        </ErrorBoundary>
      );
    ...
    if (pathname.startsWith("/profile"))
      return (
        <ErrorBoundary page="الملف الشخصي" onBack={()=>navigate("/")}>
          <ProfilePage onLogout={()=>{ setCurrentUser(null); navigate("/"); }}/>
        </ErrorBoundary>
      );
  ...
  return (
    <div dir={appDir} data-app-lang={appLang} style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      {/* ── Page content ─────────────────────────────────────────────────── */}
      <div style={(isFullDetailRoute || pathname === "/messages")
