...
export async function signUp({ email, password, name, phone }) {
  ...
  if (user?.id) {
    // إضافة طلب في pending_requests لمراجعة الأدمن
    await fetch(`${SUPABASE_URL}/rest/v1/pending_requests`, {
      method: 'POST',
    ...
    // profile بحالة pending حتى يقبله الأدمن
...
// ─── Token Refresh ───────────────────────────────────────────────────────────
// يجدد الـ access_token قبل انتهاء صلاحيته بـ 5 دقائق
let _refreshTimer = null;

export async function refreshSession() {
  const session = getSession();
  if (!SUPABASE_READY || !session?.refresh_token) return null;
  try {
    ...
    if (!res.ok) {
      // refresh_token منتهي أو ملغى — تسجيل خروج آمن
      localStorage.removeItem('sb_session');
      window.dispatchEvent(new Event('driverent-auth-change'));
      return null;
    }
  ...
  } catch { /* شبكة — سنحاول مرة أخرى */ }
  return null;
}

function _scheduleRefresh(expiresInSeconds = 3600) {
  clearTimeout(_refreshTimer);
  // جدد قبل الانتهاء بـ 5 دقائق (300 ثانية)
  const delay = Math.max((expiresInSeconds - 300) * 1000, 30_000);
  _refreshTimer = setTimeout(refreshSession, delay);
}

// بدّء الجدولة تلقائياً عند وجود session نشطة
(function initRefreshSchedule() {
  const session = safeJson(localStorage.getItem('sb_session') || 'null');
  if (session?.refresh_token && session?.expires_in) {
    // احسب الوقت المتبقي من created_at إن وجد، وإلا افترض انتهاء قريب
    const issuedAt = session.created_at ? session.created_at * 1000 : Date.now() - 1800_000;
    const expiresAt = issuedAt + (session.expires_in || 3600) * 1000;
    const remainingSecs = Math.max((expiresAt - Date.now()) / 1000, 30);
    _scheduleRefresh(remainingSecs);
  }
})();
