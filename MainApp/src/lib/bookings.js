...
// ─── حفظ محلي فقط (بدون إرسال لـ Supabase) ────────────────────────────────
function saveLocalOnly(booking) {
...
// ─── saveBooking — async حقيقي، ينتظر Supabase قبل الإعلان عن النجاح ──────
// يُرجع: { booking, synced: boolean }
// إذا فشل Supabase يُخزَّن محلياً ويُرجع synced=false (الـ caller يعرض تحذير)
export async function saveBooking(payload) {
  ...
  // 1. حفظ محلي فوري (للـ optimistic UI)
  saveLocalOnly(booking);
  window.dispatchEvent(new CustomEvent('driverent:booking-created', { detail: booking }));

  // 2. محاولة الإرسال لـ Supabase والانتظار
  try {
    const rows = await dbInsert('bookings', toRemoteBooking(booking));
    const remote = rows?.[0];
    // تحديث الـ id المحلي بـ id Supabase إن اختلف
    if (remote?.id && String(remote.id) !== String(booking.id)) {
      const updated = { ...booking, remoteId: remote.id };
      saveLocalOnly(updated);
    }
    window.dispatchEvent(new CustomEvent('driverent:booking-synced', { detail: remote || booking }));
    return { booking, synced: true };
  } catch (err) {
    console.warn('Supabase booking save failed, kept locally:', err.message);
    return { booking, synced: false, error: err.message };
  }
}
