...
export function useCars() {
  ...
  const fetchCars = useCallback(async () => {
    ...
    try {
      const rows = await dbSelect("cars", query);
      ...
      if (cached.length > 0) { setCars(cached); setOffline(true); }
      else { setError(err.message); setCars([]); }
    } finally {
      setLoading(false);
    }
  }, []); // eslint-disable-line
  ...
  return { cars, loading, error, offline, refetch: fetchCars };
}
